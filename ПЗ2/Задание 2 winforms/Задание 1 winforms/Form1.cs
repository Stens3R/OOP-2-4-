...
namespace Задание_1_winforms
{
    public partial class Form1 : Form
    {
        ...
        private void start_colection_button_Click(object sender, EventArgs e)
        {
            ...
            A.HeartBeat += (o, args) => { HeartBeatTimestamp_label.Text = $"HeartBeat Timestamp: {args.TimeStamp}"; };
            A.StartCollecting();
        }
        ...
        private void create_class_button_Click(object sender, EventArgs e)
        {
            ...
            switch (radioButton_mass_device.Checked)
            {
                case true:
                    {
                        A = new MeasureMassDevice(value, DateTime.Now.ToString().Replace(":", "-"));
                        break;
                    }
                default:
                    {
                        A = new MeasureLengthDevice(value, DateTime.Now.ToString().Replace(":", "-"));
                        break;
                    }
            }
        }
    ...
    namespace MeasuringDevice
    {
        ...
        public abstract class MeasureDataDevice : IEventEnabledMeasuringDevice
        {
            ...
            private void StartHeartBeat()
            {
                ...
                heartBeatTimer.DoWork += (o, args) => { while (true) {
                        Thread.Sleep(HeartBeatInterval);
                        if (heartBeatTimer.CancellationPending) break;
                        heartBeatTimer.ReportProgress(0);
                    }
                };
                heartBeatTimer.ProgressChanged += (object sender, ProgressChangedEventArgs e) => { OnHeartBeat(); };
        ...
        {
            private MeasureMassDevice(Units unit)
            {
                unitsToUse = unit;
                measurementType = DeviceType.MASS;
            }
            public MeasureMassDevice(Units unit, int HeartBeatInterval = 1000):this(unit)
            {
                this.HeartBeatInterval = HeartBeatInterval;
            }
            public MeasureMassDevice(Units unit, string LogFileName,int HeartBeatInterval = 1000):this(unit, HeartBeatInterval)
            {
        ...
        class MeasureLengthDevice : MeasureDataDevice
        {
            ...
            private MeasureLengthDevice(Units unit)
            {
                unitsToUse = unit;
                measurementType = DeviceType.MASS;
            }
            public MeasureLengthDevice(Units unit, int HeartBeatInterval = 1000) : this(unit)
            {
                this.HeartBeatInterval = HeartBeatInterval;
            }
            public MeasureLengthDevice(Units unit, string LogFileName, int HeartBeatInterval = 1000) : this(unit, HeartBeatInterval)
            {
                LoggingFileName = LogFileName;
            }
            public override decimal ImperialValue()
